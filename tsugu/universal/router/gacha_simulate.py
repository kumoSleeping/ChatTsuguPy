    ...
    if res.args:
        if len(res.args) == 1:
            if res.args[0].isdigit():
                ...
                return tsugu_api.gacha_simulate(user.server_mode, times=times)
            else:
            ...
            if res.args[0].isdigit() and res.args[1].isdigit():
                ...
                return tsugu_api.gacha_simulate(user.server_mode, times=times, gacha_id=gacha_id)
            else:
                return text_response('请输入正确的次数和卡池ID')
    return tsugu_api.gacha_simulate(user.server_mode)
